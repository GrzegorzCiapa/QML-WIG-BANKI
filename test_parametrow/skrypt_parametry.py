...
X_banks = X_norm[:, :5]  # Kolumny 0-4: Banki (Ry)
X_macro = X_norm[:, 5:]  # Kolumny 5-9: Makro (Rz)

NUM_QUBITS = 5
HUB_QUBIT = 1  # W tym wariancie Pekao (indeks 1) jest fizycznym HUB-em

def build_base_ansatz(x_b, x_m, num_qubits=NUM_QUBITS, hub_qubit=HUB_QUBIT):
    qc = QuantumCircuit(num_qubits)
    for i in range(num_qubits):
        qc.ry(x_b[i], i)  # Zawsze bank
        qc.rz(x_m[i], i)  # Zawsze makro
    for i in range(num_qubits):
        if i != hub_qubit:
            qc.cx(hub_qubit, i)
    return qc
...
print(f">>> Testowanie wszystkich 120 powiązań Makro -> Banki przy stałym HUB (Kubit 1: Pekao)...")

for idx, m_perm in enumerate(macro_permutations):
    # Wybieramy ułożenie zmiennych makro dla banków
    # Bank i (kubit i) dostaje cechę makro m_perm[i]
    X_m_perm = X_macro[:, m_perm]
    
    # Rzutowanie kwantowe - BATCHING (Zoptymalizowane obliczenia)
    circuits = [build_base_ansatz(X_banks[i], X_m_perm[i]) for i in range(n_samples)]
    ...
    for t in range(TRAIN_WINDOW, n_samples):
        ...
        if len(np.unique(y_tr)) > 1:
            ...
            # Zabezpieczenie przed błędem z pustą tablicą
            if len(non_zero_dists) == 0:
                gamma_val = 1.0
            else:
                gamma_val = 1.0 / (2.0 * (np.median(non_zero_dists) ** 2) + 1e-8)
    ...
    results_list.append({
        ...
        "PKO (q0)": f"PKO + {MACRO[m_perm[0]]}",
        "PEKAO_HUB (q1)": f"PEKAO + {MACRO[m_perm[1]]}",
        "SAN (q2)": f"SAN + {MACRO[m_perm[2]]}",
        "ING (q3)": f"ING + {MACRO[m_perm[3]]}",
        "MBK (q4)": f"MBK + {MACRO[m_perm[4]]}",
        "AUC ROC": auc,
        "Accuracy": acc,
        "F1-Score": f1
    })

    pd.DataFrame(results_list).to_csv("wyniki_parowania_makro.csv", index=False)
    print(f"[{idx+1}/120] Sparowano: Pekao + {MACRO[m_perm[1]]} | AUC: {auc:.4f}", flush=True)

df_res = pd.DataFrame(results_list).sort_values(by="AUC ROC", ascending=False)
df_res.to_csv("wyniki_parowania_makro_posortowane.csv", index=False)